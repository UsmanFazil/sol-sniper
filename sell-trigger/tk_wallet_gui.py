...
class WalletTokenGUI:
    ...
    def swap_via_cli(self, input_mint: str, output_mint: str, lamports: int, slippage: float):
        """
        Calls the Node.js CLI wrapper and returns the parsed result dict.
        Raises RuntimeError if swap fails.
        """
        # Construct the absolute path to the compiled Node.js CLI script
        # Fix the path - it should be relative to the current script's directory
        cli_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "sol-sniper", "js", "cli", "cli","swap-cli.js")
        )
        
        # Ensure the CLI script exists
        if not os.path.exists(cli_path):
             raise FileNotFoundError(f"Node.js CLI script not found at: {cli_path}")

        # Construct the command to execute the Node.js script
        cmd = [
            "node", # Use 'node' to execute the .js file on Windows
            cli_path,
            "--inputMint",  input_mint,
            "--outputMint", output_mint,
            "--amount",     str(lamports), # Pass amount as string
            "--slippage",   str(slippage), # Pass slippage as string
        ]
        
        logger.info(f"Executing swap CLI command: {' '.join(cmd)}")
        ...
        try:
            ...
            # Parse stdout (expected JSON result)
            result = json.loads(proc.stdout.strip())
            
            # Check the 'success' key in the JSON output
            if not result.get("success", False):
                # If success is false, raise an error with the error message from the JSON
                raise RuntimeError(result.get("error", "Unknown swap error from CLI"))
                
            # Return the parsed result dictionary on success
            return result
            
        except FileNotFoundError:
            # Handle case where 'node' command or the script path is incorrect
            raise RuntimeError(f"Node.js or the swap CLI script was not found. Ensure Node.js is in PATH and script exists at {cli_path}")
        except subprocess.CalledProcessError as e:
            # Handle errors where the Node.js script exited with a non-zero code
            # Try to parse stderr as JSON first, fallback to raw stderr
            try:
                err_output = json.loads(e.stderr.strip())
                error_message = err_output.get("error", e.stderr.strip())
            except json.JSONDecodeError:
                error_message = e.stderr.strip()
            raise RuntimeError(f"Swap CLI script failed with exit code {e.returncode}: {error_message}")
        except json.JSONDecodeError:
            # Handle errors where the stdout is not valid JSON
            raise RuntimeError(f"Failed to parse JSON output from swap CLI script. Raw output: {proc.stdout.strip()}")
        except Exception as e:
            # Catch any other unexpected errors during execution
            raise RuntimeError(f"An unexpected error occurred during CLI execution: {e}")
    ...
    def update_token_tree(self):
        ...
        try:
            ...
            for mint, token_data in self.token_data.items():
                ...
                # print("=======> we are here ",self.token_initial_prices)
                ...
                #     status = "SELLING"
                #     status_color = "blue"
                if mint in self.token_initial_prices:
    ...
    async def sell_token(self, token_mint: str, current_price: float, amount: float, reason: str) -> bool:
        """
        Sell the specified token via the Node.js CLI wrapper.
        ...
        try:
            # Resolve token name
            token_name = self.token_metadata.get(token_mint, {}).get("name", token_mint[:8])
            ...
            logger.info(f"Current value: {current_value:.6f} SOL")
            logger.info(f"Initial investment: {self.initial_buy_amount:.6f} SOL")
            profit_loss = current_value - self.initial_buy_amount
            profit_loss_pct = ((profit_loss / self.initial_buy_amount) * 100) if self.initial_buy_amount else 0
            logger.info(f"Profit/Loss: {profit_loss:.6f} SOL ({profit_loss_pct:.2f}%)")
            held_seconds = int(time.time() - self.token_entry_times.get(token_mint, time.time()))
            logger.info(f"Time in wallet: {held_seconds}s")

            # Update UI status
            self.root.after(0, lambda: self.update_status(f"SELLING: {token_name} - {reason}", "blue"))

            # Compute amount in smallest units
            decimals = self.token_metadata.get(token_mint, {}).get("decimals", 0)
            lamports = int(amount * (10 ** decimals))

            # Invoke the CLI wrapper
            res = self.swap_via_cli(
                input_mint=token_mint,
                output_mint=self.SOL_MINT,
                lamports=lamports,
                slippage=self.stop_loss_percentage / 100.0
            )

            # Validate result
            tx_ids = res.get("txIds") or []
            if not tx_ids:
                raise RuntimeError("CLI returned no transaction IDs")

            # Mark as sold and update UI
            logger.info(f"Swap succeeded, txIds: {tx_ids}")
            self.tokens_sold.add(token_mint)
            self.root.after(0, lambda: self.update_status(f"SOLD {token_name}", "green"))
            success = True

        except Exception as e:
            # Log and display failure
            error_msg = str(e)
            logger.error(f"Error selling token {token_mint}: {error_msg}")
            # Fix the lambda variable capture issue by capturing the error message
            self.root.after(0, lambda msg=error_msg: self.update_status(f"Swap failed: {msg}", "red"))

        finally:
            # Always clear the pending-sell flag
            self.pending_sells.discard(token_mint)
...
def main():
    """Main entry point"""
    wallet_address = "G4x6G8bhUTPeb4qwLxKpemuVfnqwvQmKxN9k8padE3tw"
    helius_url = "https://mainnet.helius-rpc.com/?api-key=2a5662eb-0c61-4927-b8db-4129f939fbfd"
